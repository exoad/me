...
export default function BlogPostPage() {
                ...
                <div className="min-h-screen bg-bg0 flex items-center justify-center">
                    <div className="text-center">
                        ...
                        <button
                            ...
                            className="text-fg3 hover:text-fg2 transition-colors duration-300 text-sm font-sans"
                        >
                            Back to Blog
                        </button>
                    </div>
    ...
    return (        <>            <SEO                title={post.title}                description={post.excerpt}                url={`https://exoad.net/blog/${post.slug}`}            />            <div className="min-h-screen bg-bg0">                {/* Floating TOC */}                <BlogToc entries={post.toc} />
                <div className="max-w-2xl mx-auto px-6 py-12">
                    {/* Back button */}
                    <button
                        onClick={() => navigate('/blog')}
                        className="flex items-center gap-2 text-fg4 hover:text-yellow transition-colors duration-300 text-sm font-sans mb-12 group"
                    >
                        <MdArrowBack size={16} />
                    ...
                    <article className="blog-post">
                        ...
                        <header className="mb-12">
                            <div className="flex items-baseline gap-4 mb-4">
                                ...
                                <span className="text-blue text-xs font-sans">
                                    {post.tags.map(t => `#${t}`).join(' ')}
                                </span>
                            </div>
                            <h1 className="text-3xl md:text-4xl font-bold font-playfair text-yellow mb-6">
                                {post.title}
                            </h1>
