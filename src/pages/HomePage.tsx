  ...
  return (
    <div className="relative overflow-hidden rounded-lg border border-yellow/30 bg-gradient-to-br from-bg1 to-bg0 p-6 sm:p-8">
      ...
      <div className="relative z-10">
        ...
        <h3 className="text-2xl sm:text-3xl font-bold font-playfair text-fg0 mb-3">
          Drosk
        </h3>
        <p className="text-fg3 text-sm font-sans leading-relaxed mb-4 max-w-lg">
          A modular AI-native workflow engine. Built from the ground up with a custom component system 
          that bridges local LLMs with structured automation. Privacy-first, offline-capable AI tooling 
...
function ContentSections() {
  ...
  const otherFeatured = featuredProjects.filter(p => p.title !== "Drosk" && p.title !== "bibo");
  const allOtherProjects = projects.filter(p => !p.featured && p.title !== "Drosk" && p.title !== "bibo");
  ...
  return (
    <div className="flex flex-col gap-24">
      ...
      <section>
        <h2 className="text-fg4 font-sans uppercase tracking-[0.2em] text-[10px] mb-6">Current Focus</h2>
        <DroskTeaser />
      </section>
      ...
      {biboProject && (
        <section>
          <h2 className="text-fg4 font-sans uppercase tracking-[0.2em] text-[10px] mb-6">Active Project</h2>
          <div className="border-l-2 border-green pl-4">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-fg0 text-lg font-playfair font-semibold">{biboProject.title}</span>
