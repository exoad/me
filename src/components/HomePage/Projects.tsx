    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gb-bg0-hard px-4 sm:px-8 md:px-16 py-8" ref={projectsRef}>
            <div className="flex flex-col items-center justify-center gap-6 md:gap-12 w-full max-w-6xl">
                ...
                <div
                    className={`w-[8rem] h-px bg-gb-fg2 transition-all duration-1000 ${kindaVisible ? 'opacity-100 scale-x-100' : 'opacity-0 scale-x-0'}`}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 lg:gap-10 justify-items-center">
                    {projects.filter((project, _) => project.featured).map((project, index) => (
                        <div
                            key={project.title}
                            className={`w-80 bg-gb-bg0/30 border border-gb-bg3/30 rounded-lg transition-all duration-1000 ease-out p-8 grid grid-rows-[auto_1fr_auto] min-h-[22rem] ${kindaVisible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-6 scale-95'}`}
                        ...
                        >
                            <div className="flex flex-col items-center gap-3">
                                ...
                                <p className="mt-1 text-gb-fg2 text-sm text-center font-montserrat">{project.description}</p>
                            </div>
                            <div className="flex flex-col items-center gap-3 row-start-3 mt-4">
                                ...
                                <button
                                    className="bg-transparent border-none text-gb-fg1 text-sm font-light font-montserrat py-2 px-6 cursor-pointer duration-300 group"
